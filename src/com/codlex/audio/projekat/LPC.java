...
public class LPC {
	...
	public LPC(final int coeficientCount, final List<Window> windows) {
	...
	private List<Double> calculateCoeficients(final Window window) {
		...
		// return Util.Array.toList(LPC2.getCoefficients(12, Util.Array.converToShort(window.getSamples())));
		return Util.Array.toList(coef.getColumn(0));
		
		//return Util.Array.toList(lpc_from_data(Util.Array.fromList(window.getSamples()),window.getSamples().size(),  12));
	}
